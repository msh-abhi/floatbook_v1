/*
  ...
  1. New Tables
    ...
    - `activation_keys`
      ...
      - `used_by_company_id` (uuid, nullable, references companies)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Security
    - Enable RLS on all tables
    - Add appropriate policies for superadmin and company access
    - Create SECURITY DEFINER function for key activation

  3. Functions
    - `activate_plan_with_key(activation_key text)` - Activates a plan using a key
*/
...
CREATE POLICY "Only superadmin can manage plans"
  ON plans
  FOR ALL
  TO authenticated
...
CREATE POLICY "Only superadmin can manage subscriptions"
  ON subscriptions
...
CREATE POLICY "Only superadmin can manage activation keys"
  ON activation_keys
  FOR ALL
  TO authenticated
  USING (get_system_role(auth.uid()) = 'superadmin')
