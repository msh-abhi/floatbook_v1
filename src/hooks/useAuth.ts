...
interface AuthContextType {
  ...
  refreshCompany: () => Promise<void>; // Ensure this function is part of the type
}
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchUserData = async (userId: string) => {
    try {
      const { data: userData } = await supabase.from('users').select('system_role').eq('id', userId).single();
      setSystemRole(userData?.system_role || 'user');

      if (userData?.system_role !== 'superadmin') {
        const { data: companyData } = await supabase.from('company_users').select('company_id').eq('user_id', userId).single();
